import React, { useEffect, useState } from "react";
import { useHistory } from "react-router";
import axios from "axios";

const MyCourseCard = ({ course, getMycourse }) => {
  console.log(course);

  const history = useHistory();

  const [mcourse, setMcourse] = useState([]);

  const deleteCourse = () => {
    var config = {
      method: "delete",
      url: "http://localhost:5000/usercourse/" + mcourse.ucid,
      headers: {
        Authorization: "Bearer " + sessionStorage.getItem("token"),
      },
    };

    axios(config)
      .then(function (response) {})
      .catch(function (error) {
        console.log(error);
      });
    var config1 = {
      method: "delete",
      url: "http://localhost:5000/course/" + course.courseid,
      headers: {
        Authorization: "Bearer " + sessionStorage.getItem("token"),
      },
    };

    axios(config1)
      .then(function (response) {
        getMycourse();
      })
      .catch(function (error) {
        console.log(error);
      });
  };

  useEffect(() => {
    var config = {
      method: "get",
      url: "http://localhost:5000/course/" + course.courseid,
      headers: {
        Authorization: "Bearer " + sessionStorage.getItem("token"),
      },
    };

    axios(config)
      .then(function (response) {
        setMcourse(response.data[0]);
      })
      .catch(function (error) {
        console.log(error);
      });
  }, []);

  const edit = () => {
    history.push("/create", { courseid: course.courseid });
  };

  const gotoCourse = () => {
    history.push("/course", { course: mcourse });
  };
  return (
    <div className="MyCourseCard col-md-3  my-1 p-4">
      <div className="card h-100">
        {course.publisher === course.userid ? (
          <div className="position-absolute w-100 d-flex justify-content-between">
            <button className="btn btn-outline-danger" onClick={deleteCourse}>
              <svg
                width="1em"
                height="1em"
                viewBox="0 0 16 16"
                class="bi bi-trash-fill"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fill-rule="evenodd"
                  d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5a.5.5 0 0 0-1 0v7a.5.5 0 0 0 1 0v-7z"
                />
              </svg>
            </button>
            <button className="btn btn-outline-secondary" onClick={edit}>
              <svg
                width="1em"
                height="1em"
                viewBox="0 0 16 16"
                class="bi bi-pencil-fill"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fill-rule="evenodd"
                  d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"
                />
              </svg>
            </button>
          </div>
        ) : (
          <div></div>
        )}
        <img
          className="card-img-top w-100"
          style={{ height: "50%" }}
          src={mcourse.imgurl}
          alt="a"
        />
        <div className="card-body">
          <h5 className="card-title">{mcourse.coursename}</h5>
          <p className="card-text">{mcourse.coursedescription}</p>
        </div>
        <button
          className="btn btn-outline-success m-2 text-black-50"
          onClick={gotoCourse}
        >
          Go to course
        </button>
      </div>
    </div>
  );
};

export default MyCourseCard;
